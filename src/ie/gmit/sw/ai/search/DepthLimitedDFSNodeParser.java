...
public class DepthLimitedDFSNodeParser implements Callable<WordFrequency[]> {
	...
	private Map<Integer, WordFrequency> frequencyMap = new ConcurrentHashMap<>();
	...
	public DepthLimitedDFSNodeParser(String term, File ignoreFile, File fuzzyFile, int size, String ai)
			throws IOException {
		this.es = Executors.newSingleThreadExecutor();
		this.searchTerm = term.toLowerCase().split(" ");
		this.url = "https://duckduckgo.com/html/?q=" + term;
		this.fuzzyFile = fuzzyFile;
		this.wordSize = size;
	...
	public DepthLimitedDFSNodeParser(String term, File ignoreFile, VersatileMLDataSet data, MLRegression bestMethod,
			int size, String ai) throws IOException {
		this.es = Executors.newSingleThreadExecutor();
		this.searchTerm = term.toLowerCase().split(" ");
		this.url = "https://duckduckgo.com/html/?q=" + term;
	...
	private double getHeuristicScore(Document doc) {
		...
		if (metaTags != null) {
			/*
			 * loop for all elements in meta tag extract the text content from meta tag
	...
	private void indexWords(Elements meta, String title, List<String> headings, String body) {
		...
		if (headings != null && headings.size() > 0) {
			...
			for (String s : headings) {
				...
				/*
				 * convert string to string array and loop for each word in a heading tag check
				 * for new occurrence or update existing value continue in the loop if string
				 * deemed irrelevant
				 */
	...
	private int getFrequency(String s) {
		...
		// loop for each word and check for search term, increase frequency for each
		// occurrence
	...
	private WordFrequency[] dfs(DocumentNode node, int depth, WordFrequency[] words) throws IOException {

		if (depth > limit) {
			int count = 0;
			words = new WordFrequency[frequencyMap.size()];
		...
		for (Element e : edges) {
			...
			if (closed.size() >= MAX_PAGES) {
				return Arrays.copyOfRange(words, 0, wordSize);
			}

			String link = e.absUrl("href");

			if (link != null && !closed.contains(link)) {
				try {
					closed.add(link);
					Document child = Jsoup.connect(link).get();
					double score = getHeuristicScore(child);
					queue.offer(new DocumentNode(child, score));
					words = dfs(queue.poll(), depth + 1, words);
